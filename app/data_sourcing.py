...
class Data_Sourcing:
    ...
    def intervals(self, selected_interval):
        ...
        self.period = None
        exchange_interval = {
            'Yahoo! Finance': {'5 Minute': '5m', '15 Minute': '15m', '30 Minute': '30m', '1 Hour': '60m',
                               '1 Day': '1d', '1 Week': '1wk', '1 Month': '1mo'},
            'Binance': {'1 Minute': '1m', '3 Minute': '3m', '5 Minute': '5m', '15 Minute': '15m', '30 Minute': '30m',
                        '1 Hour': '1h', '6 Hour': '6h', '12 Hour': '12h', '1 Day': '1d', '1 Week': '1w',
                        '1 Month': '1M'}}
    ...
    def apis(self, asset):
        ...
        if self.exchange != 'Yahoo! Finance':
            self.ticker_market = self.df_crypto[((self.df_crypto['Currency'] == self.asset) &
                                                 (self.df_crypto['Market'] == self.market))][
                f'{self.exchange} Pair'].values[0]
            ...
            if self.exchange == 'Binance':
                ...
                self.df['Date'] = [dt.datetime.fromtimestamp(x / 1000.0).replace(microsecond=0) for x in
                                   self.df.open_time]

        else:
            try:
                self.ticker = self.df_stocks[
                    ((self.df_stocks['Company'] == self.asset) & (self.df_stocks['Index Fund'] == self.market))][
                    'Ticker'].values[0]
                self.market = self.df_stocks[((self.df_stocks['Company'] == self.asset) &
                                              (self.df_stocks['Index Fund'] == self.market))]['Currency_Name'].unique()[
                    0]
            except:
                try:
