...
class Indications(Technical_Calculations):
    ...
    def engulfing_analysis(self):
        self.df.loc[((self.df['Adj Close'] < self.df['Open']) | (
                    self.df['Adj Close'] <= self.df['Open'].shift(self.engulfing_period)) &
                     (self.df['Adj Close'].shift(self.engulfing_period) > self.df['Open'].shift(
                         self.engulfing_period))), 'Engulfing_Indication'] = 2
        self.df.loc[((self.df['Adj Close'] > self.df['Open']) | (
                    self.df['Adj Close'] >= self.df['Open'].shift(self.engulfing_period)) &
                     (self.df['Adj Close'].shift(self.engulfing_period) < self.df['Open'].shift(
                         self.engulfing_period))), 'Engulfing_Indication'] = 0
    ...
    def stochastic_analysis(self):
        self.df.loc[((self.df['SR_K'] > self.df['SR_D']) & (self.df['SR_K'] >= 80) & (
                    self.df['RSI'] >= 70)), 'SR_Indication'] = 0
        self.df.loc[((self.df['SR_K'] < self.df['SR_D']) & (self.df['SR_K'] <= 20) & (
                    self.df['RSI'] <= 30)), 'SR_Indication'] = 2
        self.df['SR_Indication'] = self.df['SR_Indication'].fillna(1)

    def moving_average_analysis(self):
        self.df.loc[((self.df['SMA'] < self.df['LMA']) & (
                    self.df['SMA'].shift(self.sma) > self.df['LMA'].shift(self.lma))), 'MA_Indication'] = 0
        self.df.loc[((self.df['SMA'] > self.df['LMA']) & (
                    self.df['SMA'].shift(self.sma) < self.df['LMA'].shift(self.lma))), 'MA_Indication'] = 2
        self.df['MA_Indication'] = self.df['MA_Indication'].fillna(1)

    def support_resistance(self):
        self.df.loc[((self.df['SMA'] < self.df['Adj Close']) & (
                    self.df['SMA'].shift(self.sma) > self.df['Adj Close'].shift(self.sma))),
    ...
    def price_action(self):
        ...
        self.df.loc[
            ((self.df['Indication'] >= 1.25) & (self.df['Adj Close'] <= self.df['P'])), 'Distinct_Action'] = 'Buy'
        self.df.loc[
            ((self.df['Indication'] <= 0.75) & (self.df['Adj Close'] >= self.df['P'])), 'Distinct_Action'] = 'Sell'
