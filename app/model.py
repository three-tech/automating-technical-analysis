...
class Prediction(Preprocessing):
    ...
    def __init__(self, exchange, interval, asset, action_model, price_model, market=None):
        ...
        self.price_features, self.price_labels = super(Prediction, self).scaling(
            self.df_price[price_features + ['Adj Close_Scaled']])
    ...
    def get_prediction(self):
        ...
        self.model_prediction_action = np.array(
            self.ohe.inverse_transform(self.model_prediction_action.round())).flatten()
    ...
    def prediction_postprocessing(self, indication):
        ...
        self.df_visualization = self.df_visualization[
            ['Open', 'Adj Close', 'Volume', 'Distinct_Action', 'Action_Predictions', 'Price_Predictions']]

        self.df_visualization['Price_Buy'] = \
        self.df_visualization[self.df_visualization[indicators[self.indication]] == 'Buy']['Adj Close']
        self.df_visualization['Price_Sell'] = \
        self.df_visualization[self.df_visualization[indicators[self.indication]] == 'Sell']['Adj Close']

        self.df_visualization['Bullish Volume'] = \
        self.df_visualization[self.df_visualization['Adj Close'] >= self.df_visualization['Open']]['Volume']
        self.df_visualization['Bearish Volume'] = \
        self.df_visualization[self.df_visualization['Adj Close'] < self.df_visualization['Open']]['Volume']

        self.df_visualization_technical = self.df[
            ['OBV', 'MACD', 'MACDS', 'MACDH', 'RSI', 'SR_K', 'SR_D', 'SR_RSI_K', 'SR_RSI_D', 'ATR']]
